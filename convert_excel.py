...
def normalizar_setor(setor):
    ...
    if 'AMBUL' in s_upper: return 'Ambulatorio'
    if 'FINANC' in s_upper: return 'Financeiro'
    if 'COMPRA' in s_upper: return 'Compras'
    if 'JUR' in s_upper: return 'Juridico'
    if 'RH' in s_upper or 'RECURSO' in s_upper: return 'RH'
    if 'ALMOX' in s_upper: return 'Almoxarifado'
    if 'RECEP' in s_upper: return 'Recepcao'
    if 'PRESID' in s_upper: return 'Presidencia'
    ...
    if 'FARM' in s_upper: return 'Farmacia'
    if 'LABORA' in s_upper: return 'Laboratorio'
    ...
    if 'NUTRI' in s_upper: return 'Nutricao'
    if 'AUDIO' in s_upper: return 'Audiologia'
    if 'OTORRINO' in s_upper: return 'Otorrinolaringologia'
    if 'COMUNIC' in s_upper: return 'Comunicacao'
    if 'SALA' in s_upper and 'REUNI' in s_upper: return 'Sala de Reuniao'
    ...
    if 'SERV' in s_upper and 'SOCIAL' in s_upper: return 'Servico Social'
    if 'ASSES' in s_upper: return 'Assessoria'
    if 'MEDIC' in s_upper or 'CONSULT' in s_upper: return 'Consultorio Medico'
    return s.title()
...
def normalizar_material(mat):
    ...
    mapping = {
        ...
        'ROTEADOR': 'Roteador', 'ROUTER': 'Roteador', 'ROUT': 'Roteador',
        'DVR': 'DVR',
        'CAMERA': 'Camera',
        'RACK': 'Rack',
        'MICROFONE': 'Microfone',
...
print(f"Sheet1 min_row=1 first row: {[cell.value for cell in ws1[1]]}")
count1 = 0
for row in ws1.iter_rows(min_row=2, values_only=True):
    ...
    setor_raw = row[7]
    nserie = row[8]
    baixa = row[9] if len(row) > 9 else None
    ...
    if cor: parts.append(f'Cor: {str(cor).strip()}')
    if modelo: parts.append(f'Modelo: {str(modelo).strip()}')
    detalhes = ' | '.join(parts)
...
print(f"Sheet NOVO PATRIMONIO: {count1} rows processed")

# Sheet 2: ANTIGO PATRIMONIO
ws2 = wb['ANTIGO PATRIMONIO']
print(f"Sheet2 min_row=1 first row: {[cell.value for cell in ws2[1]]}")
count2 = 0
for row in ws2.iter_rows(min_row=2, values_only=True):
    ...
    if cor: parts.append(f'Cor: {str(cor).strip()}')
    if modelo: parts.append(f'Modelo: {str(modelo).strip()}')
    if situacao: parts.append(f'Conservacao: {str(situacao).strip()}')
    detalhes = ' | '.join(parts)

    nome = f'PAT-{str(numpat).strip()}'
...
print(f"Sheet ANTIGO PATRIMONIO: {count2} rows processed")
...
print(f'Total tombamentos: {len(tombamentos)}')
print(f'Total historico: {len(historico)}')
print(f'Marcas: {sorted(list(marcas_set))}')
print(f'Produtos: {sorted(list(produtos_set))}')
print(f'Setores encontrados: {sorted(set(t["setor"] for t in tombamentos))}')
print('Arquivo salvo em dados_importacao.json')
